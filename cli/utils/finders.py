...
def walk_up(current_directory):
    ...
    try:
        names = os.listdir(current_directory)
    ...
    yield current_directory, dirs, non_dirs

    new_path = os.path.realpath(os.path.join(current_directory, '..'))

    # see if we are at the top
    if new_path == current_directory:
        return
...
class ProjectScope:
    def __init__(self, top_level=False, inner_level=False, app_level=False):
        self.top_level = top_level      # project
        self.inner_level = inner_level  # project/project
        self.app_level = app_level      # project/project/app
        self.cwd = ''
    ...
    def directory(self):
        ...
        The main project directory is the directory where the `manage.py` file is located.
        For a standard django project, assume the file path is `project/manage.py`
        """

        if self.top_level:
            print(f"TOP LEVEL")
            return self.cwd
        elif self.inner_level:
            print(f"INNER LEVEL")
            return f"{self.cwd}"
        elif self.app_level:
            print(f"APP LEVEL")
    ...
    def management_file(self):
        # project/manage.py
        return os.path.join(self.directory, 'manage.py')  # if self.directory else None


levels = (
    ('manage.py',   ProjectScope(top_level=True)),
    ('asgi.py',     ProjectScope(inner_level=True)),
    ('wsgi.py',     ProjectScope(inner_level=True)),
    ('settings.py', ProjectScope(inner_level=True)),
    ('apps.py',     ProjectScope(app_level=True)),
)
...
def find_project_files(cwd):
    ...
    for file, scope in levels:
        if file in os.listdir(cwd):
            if scope.top_level:
                scope.cwd = cwd
            elif scope.inner_level or scope.app_level:
                for directory, _, files in walk_up('.'):
                    if 'manage.py' in files:
                        print(files)
                        scope.cwd = directory
                        break

            return scope.management_file
    ...
    # print(f"{path}, {project_directory}, {management_file}")
...
def get_app_name(*args, **kwargs):
    ...
    if kwargs.get('verbose'):
        Logger.log(f"Searching for AppConfig at {os.getcwd()}")
    ...
    if not base:
        if kwargs.get('verbose'):
            Logger.log(f"Cannot find for AppConfig for this app.")
